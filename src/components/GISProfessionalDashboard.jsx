...
const GISProfessionalDashboard = () => {
  ...
  const [bookmarks, setBookmarks] = useState([
    { id: 1, name: 'Delhi Metro Area', coords: { lat: 28.6139, lng: 77.2090 } },
    { id: 2, name: 'Mumbai Central', coords: { lat: 19.0760, lng: 72.8777 } },
  ...
  const [liveCoordinates, setLiveCoordinates] = useState({ lat: 20.5937, lng: 78.9629 });
  ...
  const handleShowElevation = () => {
    ...
    console.log(`🏔️ Dashboard handleShowElevation called: ${showElevation} -> ${newValue}`);
  ...
  const generateElevationProfile = async (markers) => {
    try {
      ...
      addLog(`📈 Generated elevation profile with ${elevationPoints.length} data points`);
    } catch (error) {
  ...
  const confirmSaveDistance = () => {
    if (workingMapRef.current && workingMapRef.current.saveMeasurement) {
      workingMapRef.current.saveMeasurement(measurementName || `Measurement ${Date.now()}`);
    }
  ...
  const confirmSavePolygon = () => {
    ...
    if (workingMapRef.current && workingMapRef.current.savePolygonData) {
      workingMapRef.current.savePolygonData(polygonName || `Polygon ${Date.now()}`);
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (workingMapRef.current?.map) {
            workingMapRef.current.map.setCenter({ lat: latitude, lng: longitude });
  ...
  const formatArea = (squareMeters) => {
    if (selectedUnit === 'imperial') {
      ...
      return squareMiles >= 1 ? `${squareMiles.toFixed(2)} mi²` : `${(squareMeters * 10.7639).toFixed(0)} ft²`;
    } else {
  ...
  const getMapScale = (zoom) => {
    ...
    if (selectedUnit === 'imperial') {
      ...
      return scaleDistance >= 1000 ? `${(scaleDistance / 1000).toFixed(2)} km` : `${scaleDistance.toFixed(0)} m`;
    }
  };
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ display: 'flex', flex: 1, pt: '64px' }}>
        ...
        <Slide direction="right" in={leftSidebarOpen} mountOnEnter unmountOnExit>
          <Drawer
            variant="persistent"
            anchor="left"
            ...
            sx={{
              ...
              '& .MuiDrawer-paper': {
                ...
                boxShadow: darkMode ? '2px 0 12px rgba(0,0,0,0.5)' : '2px 0 12px rgba(0,0,0,0.1)',
                top: '64px',
          ...
          >
            ...
            <Box sx={{ 
              p: 1, 
              background: `linear-gradient(135deg, ${darkMode ? 'rgb(63,81,181)' : '#1976D2'} 0%, ${darkMode ? 'rgb(48,63,159)' : '#1565C0'} 100%)`,
              color: 'white',
              textAlign: 'center',
            ...
            <Box sx={{ p: 1, overflow: 'auto', height: 'calc(100vh - 140px)' }}>
              ...
              <Paper sx={{ 
                p: 0.5, 
                mb: 0.5, 
                bgcolor: darkMode ? 'rgba(66, 165, 245, 0.1)' : alpha(theme.palette.primary.main, 0.05),
                borderRadius: 2,
                ...
                <Grid container spacing={0.5}>
                  <Grid item xs={4}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="h6" sx={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#1976D2' }}>
                        {activeLayersCount}
                      </Typography>
                      <Typography variant="caption" sx={{ fontSize: '0.6rem' }}>Layers</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={4}>
                    <Box sx={{ textAlign: 'center' }}>
                      ...
                      <Typography variant="caption" sx={{ fontSize: '0.6rem' }}>Points</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={4}>
                    <Box sx={{ textAlign: 'center' }}>
                      ...
                      <Typography variant="caption" sx={{ fontSize: '0.6rem' }}>Polygon</Typography>
                    </Box>
                  </Grid>
                </Grid>
              </Paper>
               ...
               {/* 🛠️ PROFESSIONAL TOOLS */}
                ...
                <AccordionSummary expandIcon={<ExpandMore />} sx={{ minHeight: 28, py: 0.25, '& .MuiAccordionSummary-content': { margin: '2px 0' } }}>
                  <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#1976D2', fontSize: '0.85rem' }}>
                    🛐️ Professional Tools
                  </Typography>
                </AccordionSummary>
                  ...
                  <Grid container spacing={0.5}>
                    ...
                    <Grid item xs={6}>
                      <Button
                        ...
                        onClick={() => isPolygonDrawing ? handleStopPolygonDrawing() : handleStartPolygonDrawing()}
                        disabled={!loaded}
                        sx={{
                          fontWeight: 'bold',
                          textTransform: 'none',
           ...
           {/* 🎯 DATA MANAGER */}
            ...
            <AccordionSummary expandIcon={<ExpandMore />} sx={{ minHeight: 32, py: 0.5, '& .MuiAccordionSummary-content': { margin: '4px 0' } }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#1976D2', fontSize: '0.9rem' }}>
                🎯 Data Manager
              </Typography>
            </AccordionSummary>
            <AccordionDetails sx={{ p: 0.5, pt: 0 }}>
              <Stack spacing={0.75}>
                {/* Save Actions Grid */}
                <Box sx={{ p: 1, bgcolor: alpha(theme.palette.primary.main, 0.05), borderRadius: 2 }}>
                  <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'primary.main', mb: 1, fontSize: '0.7rem' }}>
                    💾 Quick Save
                  </Typography>
                  <Grid container spacing={0.5}>
                    <Grid item xs={6}>
                      <Button
                        variant={points.length >= 2 ? 'contained' : 'outlined'}
                        fullWidth
                        size="small"
                        startIcon={<Save />}
                        ...
                        sx={{
                          ...
                          py: 0.8,
                          minHeight: 32,
                          bgcolor: points.length >= 2 ? '#4CAF50' : 'transparent',
                          borderColor: '#4CAF50',
                          color: points.length >= 2 ? 'white' : '#4CAF50',
                          '&:hover': {
                            bgcolor: points.length >= 2 ? '#2E7D32' : 'rgba(76, 175, 80, 0.08)'
                          },
                    ...
                    <Grid item xs={6}>
                      <Button
                        variant={polygonPoints.length >= 3 ? 'contained' : 'outlined'}
                        fullWidth
                        size="small"
                        startIcon={<Save />}
                        ...
                        sx={{
                          ...
                          py: 0.8,
                          minHeight: 32,
                          bgcolor: polygonPoints.length >= 3 ? '#9C27B0' : 'transparent',
                          borderColor: '#9C27B0',
                          color: polygonPoints.length >= 3 ? 'white' : '#9C27B0',
                          '&:hover': {
                            bgcolor: polygonPoints.length >= 3 ? '#7B1FA2' : 'rgba(156, 39, 176, 0.08)'
                          },
                          '&:disabled': { opacity: 0.3 }
                        }}
                      >
                ...
                <Paper sx={{ p: 1.5, bgcolor: '#f8f9fa', borderRadius: 2, border: '1px solid #e3f2fd' }}>
                  <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'text.primary', mb: 1.5, fontSize: '0.7rem', display: 'flex', alignItems: 'center' }}>
                  ...
                  <ToggleButtonGroup
                    ...
                    sx={{ mb: 1.5, height: 30 }}
                  >
                    <ToggleButton value="distance" sx={{ px: 1, py: 0.25, fontSize: '0.65rem', flex: 1 }}>
                      📏 Distances
                    </ToggleButton>
                    <ToggleButton value="polygon" sx={{ px: 1, py: 0.25, fontSize: '0.65rem', flex: 1 }}>
                      🔷 Polygons
                    </ToggleButton>
                    <ToggleButton value="both" sx={{ px: 1, py: 0.25, fontSize: '0.65rem', flex: 1 }}>
                      📊 All Data
                    </ToggleButton>
                  </ToggleButtonGroup>
           ...
           {/* 📎 UNITS & EXPORT */}
            ...
            <AccordionDetails sx={{ p: 0.5, pt: 0 }}>
              <Stack spacing={1}>
                {/* Unit Toggle */}
                <Paper sx={{ p: 1, bgcolor: alpha(theme.palette.info.main, 0.05), borderRadius: 2 }}>
                  <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <ToggleButtonGroup
                      ...
                      sx={{ height: 28 }}
                    >
                      <ToggleButton value="metric" sx={{ px: 1, py: 0.5, fontSize: '0.7rem' }}>
                        Metric
                      </ToggleButton>
                      <ToggleButton value="imperial" sx={{ px: 1, py: 0.5, fontSize: '0.7rem' }}>
                        Imperial
                      </ToggleButton>
                    </ToggleButtonGroup>
                  </Stack>
                ...
                <ButtonGroup variant="outlined" fullWidth size="small">
                  <Button 
                    ...
                    onClick={() => {
                      const exportData = {
                        ...
                        measurements: {
                          distance: { value: totalDistance, formatted: `${(totalDistance/1000).toFixed(2)} km`, points: points.length },
                          area: { value: polygonArea, formatted: `${(polygonArea/1000000).toFixed(2)} km²`, points: polygonPoints.length }
                        },
                        coordinates: points.concat(polygonPoints),
                        metadata: { layers: activeLayers, baseMap: selectedBaseMap }
                      };
                  ...
                  <Button 
                    startIcon={<Upload />}
                    onClick={() => {
                      const csvHeader = 'Type,Name,Latitude,Longitude,Value,Unit\n';
                      const csvRows = [];
                      points.forEach((point, index) => {
                        csvRows.push(`Distance,Point ${index+1},${point.lat},${point.lng},${totalDistance},meters`);
                      });
                      polygonPoints.forEach((point, index) => {
                        csvRows.push(`Polygon,Point ${index+1},${point.lat},${point.lng},${polygonArea},square_meters`);
                      });
                      ...
                      >
                        <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                          ...
                          <Box sx={{ textAlign: 'left' }}>
                            <Typography variant="body2" fontWeight="bold" textTransform="none" sx={{ fontSize: '0.75rem' }}>
                              {map.name}
                            </Typography>
                            <Typography 
                              variant="caption" 
              ...
              <Stack direction="row" spacing={0.5}>
                <Button
                  ...
                  sx={{
                    ...
                    '&:hover': {
                      backgroundColor: showDebugLogs ? '#388E3C' : 'rgba(108, 117, 125, 0.1)',
           ...
           {/* 📖 BOOKMARKS */}
            ...
            <AccordionDetails sx={{ p: 0.5, pt: 0 }}>
              <Stack spacing={1}>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const newBookmark = {
                      ...
                      coords: { lat: liveCoordinates.lat, lng: liveCoordinates.lng },
                      timestamp: new Date().toLocaleString(),
                      zoom: mapZoom
                    };
                    setBookmarks(prev => [...prev, newBookmark]);
                    console.log(`🔖 Bookmark saved at: ${liveCoordinates.lat.toFixed(6)}, ${liveCoordinates.lng.toFixed(6)}`);
                  }}
                  sx={{
                    textTransform: 'none',
                    fontSize: '0.7rem',
                ...
                {bookmarks.length === 0 ? (
                  <Paper sx={{ p: 2, bgcolor: alpha(theme.palette.info.main, 0.05), borderRadius: 2, textAlign: 'center' }}>
                    <Bookmark sx={{ fontSize: 32, color: 'info.main', mb: 1 }} />
                    <Typography variant="body2" color="text.secondary">
                      No bookmarks yet. Click "Add Current View" to save locations.
                    </Typography>
                  </Paper>
                ) : (
                  <Stack spacing={0.5}>
                    {bookmarks.map((bookmark) => (
                      <Paper 
                        ...
                        onClick={() => {
                          console.log(`🎯 Navigating to bookmark: ${bookmark.name}`);
                          console.log(`🗺 Coordinates: ${bookmark.coords.lat}, ${bookmark.coords.lng}`);
                          // Navigate to the exact bookmark location
                          if (workingMapRef.current?.map) {
                            workingMapRef.current.map.panTo(bookmark.coords);
                            workingMapRef.current.map.setZoom(bookmark.zoom || 12);
                            console.log(`✅ Map navigated to bookmark location`);
                          } else {
                            console.error('❌ WorkingMapRef not available for navigation');
                          }
                        }}
                      >
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            ...
                            <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem' }}>
                              {bookmark.coords.lat.toFixed(4)}°, {bookmark.coords.lng.toFixed(4)}°
                            </Typography>
                          </Box>
                          <Stack direction="row" spacing={0.5}>
        ...
        >
          ...
          {(totalDistance > 0 || polygonArea > 0) && (
            <Paper
              sx={{
                ...
                p: 2,
                bgcolor: darkMode ? 'rgba(0, 0, 0, 0.9)' : 'rgba(255, 255, 255, 0.98)',
                color: darkMode ? '#fff' : 'inherit',
                backdropFilter: 'blur(20px)',
                zIndex: 1000,
                border: `2px solid ${totalDistance > 0 ? 'rgba(76, 175, 80, 0.8)' : 'rgba(156, 39, 176, 0.8)'}`,
                borderRadius: 3,
            ...
            >
              <Stack spacing={1}>
                ...
                {totalDistance > 0 && (
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: darkMode ? 'rgba(25, 118, 210, 0.1)' : 'rgba(25, 118, 210, 0.05)', borderRadius: 2 }}>
                    <Typography variant="caption" sx={{ fontSize: '0.8rem', color: 'primary.main', fontWeight: 'bold' }}>
                      DISTANCE
                    </Typography>
                    <Typography variant="h5" sx={{ fontWeight: 'bold', color: 'primary.main', fontFamily: 'monospace' }}>
                ...
                {polygonArea > 0 && (
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: darkMode ? 'rgba(156, 39, 176, 0.1)' : 'rgba(156, 39, 176, 0.05)', borderRadius: 2 }}>
                    <Typography variant="caption" sx={{ fontSize: '0.8rem', color: 'secondary.main', fontWeight: 'bold' }}>
                      AREA
                    </Typography>
                    <Typography variant="h5" sx={{ fontWeight: 'bold', color: 'secondary.main', fontFamily: 'monospace' }}>
                ...
                <Stack direction="row" spacing={1} justifyContent="center">
                  <Chip 
                    label={`Unit: ${selectedUnit === 'metric' ? 'Metric' : 'Imperial'}`}
                    size="small"
                    variant="outlined"
          ...
          <Paper
            sx={{
              ...
              left: 16,
              p: 1,
              bgcolor: darkMode ? 'rgba(0, 0, 0, 0.85)' : 'rgba(255, 255, 255, 0.95)',
              color: darkMode ? '#fff' : 'inherit',
              zIndex: 1000,
              borderRadius: 2
            }}
          ...
          >
            ...
            {hoverCoordinates && (
              <>
                <br />
                <Typography variant="caption" sx={{ fontWeight: 'bold', display: 'block', mt: 1, color: 'primary.main' }}>
                  💁 Mouse Position
                </Typography>
                <Typography variant="caption" sx={{ fontFamily: 'monospace', fontSize: '0.75rem', color: 'primary.main' }}>
              ...
              >
                {debugLogs.length === 0 ? (
                  ...
                  debugLogs.slice(-20).map((log, index) => (
                    <Typography
                      key={`debug-log-${debugLogs.length - 20 + index}-${index}`}
                      variant="caption"
                      sx={{
                        display: 'block',
                        color: log.includes('❌') ? '#FF5722' : 
                               log.includes('⚠️') ? '#FF9800' : 
                               log.includes('✅') ? '#4CAF50' : '#00FF00',
                        fontSize: '0.7rem',
                        fontFamily: 'monospace',
                        lineHeight: 1.3,
                        mb: 0.25
              ...
              <Box sx={{ p: 2, height: '100%' }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
                  ...
                  <Stack direction="row" spacing={1}>
                    <Chip 
                      label={`Distance: ${elevationData.length > 0 ? formatDistance(elevationData[elevationData.length - 1]?.distance || 0) : '0'}`}
                      size="small"
                      sx={{ bgcolor: darkMode ? 'rgba(25, 118, 210, 0.2)' : 'rgba(25, 118, 210, 0.1)' }}
                    />
                    <Chip 
                      label={`Max Elevation: ${Math.max(...elevationData.map(d => d.elevation))}m`}
                      size="small"
                      sx={{ bgcolor: darkMode ? 'rgba(76, 175, 80, 0.2)' : 'rgba(76, 175, 80, 0.1)' }}
                    />
                    <IconButton
                      size="small"
                      onClick={() => {
                ...
                <Box sx={{ 
                  ...
                  {elevationData.map((point, index) => {
                    ...
                    const heightPercentage = ((point.elevation - minElevation) / (maxElevation - minElevation)) * 100;
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          bgcolor: `hsl(${120 - (heightPercentage * 0.8)}, 70%, 50%)`,
                          mx: 0.1,
                        ...
                        title={`Distance: ${formatDistance(point.distance)}, Elevation: ${point.elevation}m`}
                      />
                    );
                  })}
                </Box>
                
                <Stack direction="row" justifyContent="space-between" sx={{ mt: 1, fontSize: '0.8rem', color: 'text.secondary' }}>
                  <Typography>Start</Typography>
                  <Typography>Distance: {elevationData.length > 0 ? formatDistance(elevationData[elevationData.length - 1]?.distance || 0) : '0'}</Typography>
        ...
        <Slide direction="left" in={rightSidebarOpen} mountOnEnter unmountOnExit>
          <Drawer
            variant="persistent"
            anchor="right"
          ...
          >
            ...
            <Box sx={{ 
              p: 1, 
              background: `linear-gradient(135deg, ${darkMode ? 'rgb(156,39,176)' : '#9C27B0'} 0%, ${darkMode ? 'rgb(123,31,162)' : '#7B1FA2'} 100%)`,
              color: 'white',
              textAlign: 'center',
              borderBottom: `1px solid ${darkMode ? 'grey.700' : '#e3f2fd'}`
            }}>
              ...
              <Paper sx={{ 
                ...
                <Stack spacing={1}>
                  <Box>
                    ...
                    <Typography variant="body2">{points.length} active</Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">Polygon Points:</Typography>
                    <Typography variant="body2">{polygonPoints.length} active</Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">Map Zoom:</Typography>
                  ...
                  <Box>
                    ...
                    <Typography variant="caption" fontFamily="monospace">
                      {liveCoordinates.lat.toFixed(6)}, {liveCoordinates.lng.toFixed(6)}
                    </Typography>
                  </Box>
                </Stack>
              </Paper>
      ...
      {/* Save Polygon Dialog */}
      <Dialog open={polygonSaveDialogOpen} onClose={() => setPolygonSaveDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ 
      ...
      <Dialog open={bookmarkEditDialogOpen} onClose={() => setBookmarkEditDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ 
      ...
      <Dialog open={bookmarkDeleteDialogOpen} onClose={() => setBookmarkDeleteDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ 
        ...
        <DialogContent sx={{ pt: 3 }}>
          ...
          {bookmarkToDelete && (
            <Paper sx={{ p: 2, bgcolor: 'rgba(244, 67, 54, 0.1)', borderRadius: 2 }}>
              <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold' }}>
                {bookmarkToDelete.name}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {bookmarkToDelete.coords.lat.toFixed(6)}°, {bookmarkToDelete.coords.lng.toFixed(6)}°
              </Typography>
